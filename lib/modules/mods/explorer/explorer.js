...
function Explorer(app) {
  ...
  // var serverAddress = 'http://localhost:12101';
  // var socket = io(serverAddress);
  // this.app.socket = socket;

  console.log('EXPLORER MODULE <------------------------------------')
...
Explorer.prototype.onNewBlock = function onNewBlock(blk) {

  var explorer_self = this;

  console.log('EXPLORER MODULE: On NEW BLOCK!!!!! <------------------------------------')
  console.log(explorer_self)
  // this.socket.emit('explorer/block/new','YO! blokkk');
...
Explorer.prototype.webServer = function webServer(app, expressapp) {
  ...
  const pathPublicAssets = '/../../../../public/'
  expressapp.use(
    '/explorer-refactor',
    serveStatic(`${__dirname}${pathPublicAssets}/explorer-refactor/dist`)
  )

  expressapp.get('/explorer/json/blocks', function (req, res) {
    const data = explorer_self.jsonBlocks();
  ...
  expressapp.get('/explorer/json/block', function (req, res) {
    explorer_self.jsonBlock(req, res);
  });

  expressapp.get('/explorer/json/transactions', function (req, res) {
    explorer_self.jsonTransactions(req, res);
  });
...
Explorer.prototype.jsonBlocks = function jsonBlocks() {
  const explorer_self = this;
  const maximumBlockAmount = 200;
  const latestBlocks = explorer_self.app.blockchain.blocks.slice(0, maximumBlockAmount);
  const longestchainhash = explorer_self.app.blockchain.index.hash[explorer_self.app.blockchain.longestChain];
  let data  = [];
  
  latestBlocks.forEach( block => {
    data.push({
      'id': block.block.id,
      'hash': block.returnHash('hex'),
      'prevHash': block.block.prevhash,
      'isLongest': longestchainhash == block.returnHash()
    })
  })
...
Explorer.prototype.jsonBlock = function jsonBlock(req, res) {
  const app = this.app;
  const bid  = req.query.bid;
  const hash = req.query.hash;

  const sendResult = function(data) {
    res.header("Access-Control-Allow-Origin", "*");
    res.header("Access-Control-Allow-Headers", "Origin, X-Requested-With, Content-Type, Accept");
    res.setHeader("Content-Type", "application/json");
    res.send(JSON.stringify(data));
  ...
  if (bid == null && hash == null) sendResult('NO BLOCK FOUND - bid & hash missing');
  ...
  const getBlock = function(app, data) {
    sendResult(data.block)
  }

  const findRow = function(err, row) {
    if (row == null) {
      sendResult('NO BLOCK FOUND - row not found');
    } else {
      const filename = `${row.block_id}-${row.id}.blk`;
      
      app.storage.openBlockByFilename(filename, getBlock);
    }
  }

  app.storage.queryDatabase(sql, params, findRow)
}

Explorer.prototype.jsonTransactions = function jsonTransactions(req, res) {
  const transactions = JSON.parse(req.query.transactions);
  let data  = [];
  ...
  transactions.forEach( tx => {
    const tmptx = new saito.transaction(tx);
    
    data.push({
      'id': tmptx.transaction.id,
      'address': tmptx.transaction.from[0].add,
      'amount': tmptx.returnAmountTo(tmptx.transaction.to[0].add),
      'fee': tmptx.returnFeeTotal(),
      'goldenticket': !!tmptx.transaction.gt,
    })
  })
...
Explorer.prototype.listTransactions = function listTransactions(blk) {
  ...
  if (blk.block.transactions.length > 0) {
    ...
    for (var mt = 0; mt < blk.block.transactions.length; mt++) {
      ...
      html += '<td><a href="/explorer/transaction?tid='+tmptx.transaction.id+'">'+tmptx.transaction.from[0].add+'</a></td>';
