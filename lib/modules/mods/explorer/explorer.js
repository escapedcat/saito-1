...
Explorer.prototype.webServer = function webServer(app, expressapp) {
  ...
  expressapp.get('/explorer/json/block', function (req, res) {
    const data = explorer_self.jsonBlock(req, res);
    // res.header("Access-Control-Allow-Origin", "*");
    // res.header("Access-Control-Allow-Headers", "Origin, X-Requested-With, Content-Type, Accept");
    // res.setHeader("Content-Type", "application/json");
    // res.send(JSON.stringify(data));
...
Explorer.prototype.jsonBlocks = function jsonBlocks() {
  ...
  for (var mb = explorer_self.app.blockchain.blocks.length-1; mb >= 0 && mb > explorer_self.app.blockchain.blocks.length-200; mb--) {
    ...
    var longestchainhash = explorer_self.app.blockchain.index.hash[explorer_self.app.blockchain.longestChain];
    const blocks = explorer_self.app.blockchain.blocks[mb]

    data.push({
      'id': blocks.block.id,
      'hash': blocks.returnHash('hex'),
      'prevHash': blocks.block.prevhash,
      'isLongest': longestchainhash == blocks.returnHash()
    })
  }
